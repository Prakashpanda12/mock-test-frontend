  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <header className="bg-gray-900 text-white shadow-md z-10 py-3 md:py-4 px-4 md:px-6 flex flex-wrap justify-between items-center gap-3 md:gap-4">
        <h1 className="text-lg md:text-xl font-bold uppercase tracking-wider w-full sm:w-auto text-center sm:text-left">OSSSC Admin Control</h1>
              ...
              <table className="w-full text-left border-collapse text-xs md:text-sm min-w-[700px]">
                <thead>
                  <tr className="bg-gray-100 border-b border-gray-200 text-gray-700">
                    ...
                    <th className="p-3 md:p-4 font-bold text-center">Questions (Up/Exp)</th>
                ...
                <tbody>
                  {exams?.map(exam => (
                    <tr key={exam._id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="p-3 md:p-4 font-mono font-semibold text-gray-800">{exam.examId}</td>
                      <td className="p-3 md:p-4 font-semibold text-osssc-blue">{exam.title}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="bg-gray-900 text-white px-4 md:px-6 py-3 md:py-4 flex justify-between items-center shrink-0">
              <h2 className="text-base md:text-lg font-bold">Create New Examination</h2>
            ...
            <form onSubmit={handleCreate} className="p-4 md:p-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <input type="number" value={durationMinutes} onChange={(e) => setDurationMinutes(e.target.value)} required className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-osssc-blue text-sm" />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-bold text-gray-700 mb-1 md:mb-2">Default Negative Mark</label>
                  <input type="number" step="0.01" value={negativeMarking} onChange={(e) => setNegativeMarking(e.target.value)} required className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-osssc-blue text-sm" />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-bold text-gray-700 mb-1 md:mb-2">Total Questions</label>
                  <input type="number" value={totalQuestions} onChange={(e) => setTotalQuestions(e.target.value)} required className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-osssc-blue text-sm" />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-bold text-gray-700 mb-1 md:mb-2">Total Marks</label>
                  <input type="number" value={totalMarks} onChange={(e) => setTotalMarks(e.target.value)} required className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-osssc-blue text-sm" />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={createMutation.isPending} className="bg-osssc-blue hover:bg-blue-800 text-white font-bold py-2 px-6 rounded shadow disabled:opacity-50">
